...
@router.post("/bootstrap/users/{email}/make-admin", response_model=UserResponse)
def bootstrap_make_admin(
    ...
    2. Call this endpoint with that email to set role='admin'.

    WARNING: Do NOT expose this in a real production environment.
    """
    user = UserRepository.update_role_by_email(db, email, "admin")
    if not user:
